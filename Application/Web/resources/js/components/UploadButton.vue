...
<template>
    <Dialog v-model:open="open">
        <DialogTrigger as-child>
            <Button :variant="variant" size="default" :class="triggerClass">
                <Upload class="size-4" />
                {{ label }}
            </Button>
        </DialogTrigger>
        ...
        <DialogContent class="sm:max-w-xl">
            ...
            >
                <div class="flex flex-col items-center gap-2">
                    ...
                    <p class="text-xs text-muted-foreground">
                        DOC, PDF, XLSX, PPT formats, up to 50 MB
                    </p>
                    <input
                        ref="fileInput"
                        type="file"
                        ...
                        @change="onChange"
                    />
                    <Button class="mt-2" @click="openPicker"
                        >Browse Files</Button
                    >
                </div>
            </div>
            ...
            <div v-if="uploads.length" class="mt-4 space-y-2">
                ...
                <ul class="max-h-56 space-y-1 overflow-auto">
                    <li
                        v-for="u in uploads"
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="ml-2 text-xs text-muted-foreground"
                                    >{{
                                        (u.file.size / 1024 / 1024).toFixed(1)
                                    }}
                                    MB</span
                                >
                            </div>
